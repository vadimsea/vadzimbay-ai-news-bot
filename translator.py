...
Пиши конкретно: кто что выпустил/запустил/изменил и кому это пригодится.
...
def _normalize_post_shape(text: str) -> str:
    ...
    if not any(line.strip().lower().startswith("почему это важно") for line in lines):
        ...
        why = "Это помогает понять, какие AI-инструменты уже можно применить в работе."
        if len(paragraphs) >= 2 and paragraphs[-1].lower().startswith(
            ("это поможет", "это удобно", "это важно", "такой подход", "новые функции помогут")
        ):
            why = paragraphs.pop()
            body = "\n\n".join(paragraphs)
        text = f"{body}\n\nПочему это важно:\n{why}\n\n{source}".strip()
        ...
        text = "\n".join(lines).strip()
    return _improve_why_section(text)
