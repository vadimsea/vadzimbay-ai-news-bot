...
Пиши не как переводчик, а как редактор: сначала суть события, потом 1–2 конкретные детали.
Не используй общие фразы вроде "это важно для будущего технологий", "открывает новые возможности", "меняет правила игры".
Не пересказывай пресс-релиз. Делай текст таким, чтобы его хотелось переслать коллеге.
Блок "Почему это важно" должен быть конкретным: кому и чем это может быть полезно.
...
Начни с главного события, убери вводные и общие слова.
В "Почему это важно" напиши конкретную пользу для бизнеса, разработки, дизайна, маркетинга, науки или обычных пользователей.
