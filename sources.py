from __future__ import annotations

from dataclasses import dataclass


@dataclass(frozen=True)
class NewsSource:
    name: str
    url: str
    language: str
    category: str
    trust_score: float = 1.0
    enabled: bool = True


RSS_SOURCES: list[NewsSource] = [
    NewsSource("TechCrunch AI", "https://techcrunch.com/category/artificial-intelligence/feed/", "en", "ai", 0.88),
    NewsSource("The Verge AI", "https://www.theverge.com/rss/ai-artificial-intelligence/index.xml", "en", "ai", 0.82),
    NewsSource("MIT Technology Review", "https://www.technologyreview.com/feed/", "en", "technology", 0.92),
    NewsSource("Ars Technica AI", "https://arstechnica.com/ai/feed/", "en", "ai", 0.86),
    NewsSource("Ars Technica", "https://feeds.arstechnica.com/arstechnica/index", "en", "technology", 0.86, enabled=False),
    NewsSource("VentureBeat AI", "https://venturebeat.com/category/ai/feed/", "en", "ai", 0.82),
    NewsSource("Wired", "https://www.wired.com/feed/rss", "en", "technology", 0.9),
    NewsSource("The Decoder", "https://the-decoder.com/feed/", "en", "ai", 0.88),
    NewsSource("The Gradient", "https://thegradient.pub/rss/", "en", "ai", 0.82),
    NewsSource("Last Week in AI", "https://lastweekin.ai/feed/", "en", "ai", 0.78),
    NewsSource("Ahead of AI", "https://magazine.sebastianraschka.com/feed", "en", "ai", 0.82),
    NewsSource("Latent Space", "https://www.latent.space/feed", "en", "ai", 0.82),
    NewsSource("Import AI", "https://jack-clark.net/feed/", "en", "ai", 0.78),
    NewsSource("TensorFeed", "https://tensorfeed.ai/feed.xml", "en", "ai", 0.78, enabled=False),
    NewsSource("Planet AI", "https://www.planet-ai.net/rss.xml", "en", "ai", 0.76, enabled=False),
    NewsSource("OnAI", "https://onai2.com/rss.xml", "en", "ai", 0.76, enabled=False),
    NewsSource("InfoQ AI", "https://feed.infoq.com/artificial_intelligence/news", "en", "ai", 0.82, enabled=False),
    NewsSource("Google AI Blog", "https://blog.google/technology/ai/rss/", "en", "ai", 0.92),
    NewsSource("Hugging Face Blog", "https://huggingface.co/blog/feed.xml", "en", "ai", 0.86),
    NewsSource("Product Hunt", "https://www.producthunt.com/feed", "en", "ai", 0.78, enabled=False),
    NewsSource("Smashing Magazine", "https://www.smashingmagazine.com/feed/", "en", "web_design", 0.86),
    NewsSource("UX Collective", "https://uxdesign.cc/feed", "en", "web_design", 0.58, enabled=False),
    NewsSource("Webflow Blog", "https://webflow.com/blog/rss.xml", "en", "web_design", 0.82),
    NewsSource("Figma Blog", "https://www.figma.com/blog/feed/atom.xml", "en", "web_design", 0.9),
    NewsSource("CSS-Tricks", "https://css-tricks.com/feed/", "en", "frontend", 0.82),
    NewsSource("Simon Willison", "https://simonwillison.net/atom/everything/", "en", "ai", 0.84, enabled=False),
    NewsSource("A List Apart", "https://alistapart.com/main/feed/", "en", "web_design", 0.82, enabled=False),
    NewsSource("Search Engine Land", "https://searchengineland.com/feed", "en", "marketing", 0.72),
    NewsSource("Search Engine Journal", "https://www.searchenginejournal.com/feed/", "en", "marketing", 0.82),
    NewsSource("Marketing AI Institute", "https://www.marketingaiinstitute.com/blog/rss.xml", "en", "marketing_ai", 0.84),
    NewsSource("MarTech", "https://martech.org/feed/", "en", "marketing", 0.82),
    NewsSource("HubSpot Marketing", "https://blog.hubspot.com/marketing/rss.xml", "en", "marketing", 0.78, enabled=False),
    NewsSource("IEEE Spectrum Robotics", "https://spectrum.ieee.org/rss/robotics/fulltext", "en", "robotics", 0.9, enabled=False),
    NewsSource("ScienceDaily Robotics", "https://www.sciencedaily.com/rss/computers_math/robotics.xml", "en", "robotics", 0.68, enabled=False),
    NewsSource("MarkTechPost", "https://www.marktechpost.com/feed/", "en", "ai", 0.62, enabled=False),
    NewsSource("Google DeepMind Blog", "https://deepmind.google/blog/rss.xml", "en", "ai", 0.95),
    NewsSource("OpenAI Blog", "https://openai.com/news/rss.xml", "en", "ai", 0.96),
    NewsSource("Anthropic News", "https://rsshub.bestblogs.dev/anthropic/news", "en", "ai", 0.94),
    NewsSource("NVIDIA Blog", "https://blogs.nvidia.com/feed/", "en", "technology", 0.88),
    NewsSource("Heise Online", "https://www.heise.de/rss/heise-atom.xml", "de", "technology", 0.84),
    NewsSource("Golem.de", "https://rss.golem.de/rss.php?feed=RSS2.0", "de", "technology", 0.82),
    NewsSource("t3n", "https://t3n.de/rss.xml", "de", "technology", 0.78),
    NewsSource("OMR", "https://omr.com/de/feed", "de", "marketing", 0.8, enabled=False),
    NewsSource("ComputerBase", "https://www.computerbase.de/rss/news.xml", "de", "technology", 0.8),
    NewsSource("Habr AI", "https://habr.com/ru/rss/hubs/artificial_intelligence/articles/", "ru", "ai", 0.78),
    NewsSource("Habr Robotics", "https://habr.com/ru/rss/hubs/robotics/articles/", "ru", "robotics", 0.76, enabled=False),
    NewsSource("3DNews", "https://3dnews.ru/news/rss/", "ru", "technology", 0.76, enabled=False),
    NewsSource("iXBT", "https://www.ixbt.com/export/news.rss", "ru", "technology", 0.72, enabled=False),
    NewsSource("N+1", "https://nplus1.ru/rss", "ru", "science_tech", 0.82, enabled=False),
    NewsSource("vc.ru", "https://vc.ru/rss/all", "ru", "technology", 0.68, enabled=False),
    # Development: frontend, backend, Python. Quality over volume; junk is filtered in ranker.py.
    NewsSource("GitHub Blog", "https://github.blog/feed/", "en", "dev", 0.84),
    NewsSource("Stack Overflow Blog", "https://stackoverflow.blog/feed/", "en", "dev", 0.8),
    NewsSource("JetBrains Blog", "https://blog.jetbrains.com/feed/", "en", "dev", 0.8),
    NewsSource("InfoQ", "https://feed.infoq.com/", "en", "dev", 0.78),
    NewsSource("The New Stack", "https://thenewstack.io/feed/", "en", "backend", 0.72),
    NewsSource("Cloudflare Blog", "https://blog.cloudflare.com/rss/", "en", "backend", 0.85),
    NewsSource("Go Blog", "https://go.dev/blog/feed.atom", "en", "backend", 0.85),
    NewsSource("Node.js Blog", "https://nodejs.org/en/feed/blog.xml", "en", "backend", 0.85),
    NewsSource("Python Insider", "https://blog.python.org/feeds/posts/default?alt=rss", "en", "python", 0.85),
    NewsSource("Django Weblog", "https://www.djangoproject.com/rss/weblog/", "en", "python", 0.8),
    NewsSource("Vercel Blog", "https://vercel.com/atom", "en", "frontend", 0.8),
    NewsSource("Chrome for Developers", "https://developer.chrome.com/static/blog/feed.xml", "en", "frontend", 0.88),
    NewsSource("web.dev", "https://web.dev/static/blog/feed.xml", "en", "frontend", 0.85),
    NewsSource("React Blog", "https://react.dev/rss.xml", "en", "frontend", 0.9),
    NewsSource("Mozilla Hacks", "https://hacks.mozilla.org/feed/", "en", "frontend", 0.82),
    NewsSource("Hacker News", "hn-algolia://?min_points=100&hours=72", "en", "dev_mixed", 0.82),
    NewsSource("Lobsters", "https://lobste.rs/rss", "en", "dev", 0.78),
    NewsSource("Habr Python", "https://habr.com/ru/rss/hubs/python/articles/?score=25", "ru", "python", 0.72),
    NewsSource("Habr Web Dev", "https://habr.com/ru/rss/hubs/webdev/articles/?score=25", "ru", "frontend", 0.72),
    NewsSource("Habr Best", "https://habr.com/ru/rss/best/daily/", "ru", "dev_mixed", 0.75),
    # Design
    NewsSource("Creative Bloq", "https://www.creativebloq.com/feeds/all", "en", "design", 0.78),
    NewsSource("Sidebar", "https://sidebar.io/feed.xml", "en", "design", 0.8),
    NewsSource("Nielsen Norman Group", "https://www.nngroup.com/feed/rss/", "en", "design", 0.8),
    NewsSource("Design Week", "https://www.designweek.co.uk/feed/", "en", "design", 0.75),
    NewsSource("Habr Design", "https://habr.com/ru/rss/hubs/design/articles/?score=25", "ru", "design", 0.7),
    # Marketing
    NewsSource("Social Media Today", "https://www.socialmediatoday.com/feeds/news/", "en", "marketing", 0.78),
    NewsSource("Marketing Dive", "https://www.marketingdive.com/feeds/news/", "en", "marketing", 0.78),
    NewsSource("PPC Land", "https://ppc.land/rss/", "en", "marketing", 0.7),
    NewsSource("TechCrunch Social", "https://techcrunch.com/category/social/feed/", "en", "marketing", 0.82),
    NewsSource("TechCrunch Apps", "https://techcrunch.com/category/apps/feed/", "en", "marketing", 0.8),
    NewsSource("The Verge YouTube", "https://www.theverge.com/rss/youtube/index.xml", "en", "marketing", 0.8),
    NewsSource("Marketing Week", "https://www.marketingweek.com/feed/", "en", "marketing", 0.72),
    NewsSource("Campaign", "https://www.campaignlive.com/rss/news", "en", "marketing", 0.72),
    NewsSource("Adweek", "https://www.adweek.com/feed/", "en", "marketing", 0.75),
    NewsSource("Google Search Central Blog", "https://developers.google.com/search/blog/feed.xml", "en", "marketing", 0.9),
    # Chinese and Hebrew RSS feeds can be added here later without changing main.py.
    # Example:
    # NewsSource("Example Chinese AI Feed", "https://example.com/rss", "zh", "ai", 0.7, enabled=False),
    # NewsSource("Example Hebrew Tech Feed", "https://example.com/rss", "he", "technology", 0.7, enabled=False),
]


def get_sources(enabled_only: bool = True) -> list[NewsSource]:
    if enabled_only:
        return [source for source in RSS_SOURCES if source.enabled]
    return list(RSS_SOURCES)
