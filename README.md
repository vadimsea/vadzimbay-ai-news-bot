...
MVP-проект для автоматического Telegram-канала о нейтральных технологических новостях: ИИ, нейросети, робототехника, гаджеты, автоматизация, веб-дизайн, тренды веб-дизайна, AI-маркетинг, вайбкодинг, IT-стартапы и будущее технологий.
...
- `ranker.py` — отбор новостей: жёсткие стоп-фильтры, предварительный рейтинг и выбор лучших по оценке LLM.
- `popularity.py` — очки Hacker News как сигнал живого интереса к новости.
- `llm_ranker.py` — оценка кандидатов моделью по шкале 1-10.
...
Для рекламных постов есть отдельный workflow:

```text
.github/workflows/vadzimbay-promo.yml
```

Он проверяет промо каждый день в `12:00` и `15:00` по Минску. Расписание кампаний задаётся в `promo.py`: **ATEN** по вторникам, канал удалённой работы по средам раз в 2 недели, бот-помощник программиста по пятницам. Всего 2–3 рекламных поста в неделю, чтобы реклама не перебивала новости.
...
Бот отбирает новости по четырём темам, целевая доля в выдаче: ИИ 40%, разработка (frontend, backend, Python) 25%, маркетинг 20%, дизайн 15% (`topics.py`, `TOPIC_SHARES`). Тема определяется категорией источника (`ai`, `dev`, `frontend`, `backend`, `python`, `design`, `marketing`), а для смешанных источников (Hacker News, Habr Best, `technology`) по ключевым словам.

Доли мягкие: решает качество. Пул для LLM собирается по темам, чтобы ИИ не вытеснял остальное, а каждой теме гарантируется минимум, если у неё есть новости с оценкой не ниже `TOPIC_MIN_LLM_SCORE` (по умолчанию 5). Источники для новых тем добавляются в `sources.py` с категориями выше. Hacker News подключён как источник (`hn-algolia://`): берутся истории с 100+ очками за 72 часа.

## 12.1. Как выбираются новости

1. Жёсткие фильтры: дубли, старые, политика, не по теме, реклама (`Anzeige`, `Sponsored`), гайды, подкасты, корпоративная безопасность.
2. Предварительный рейтинг: доверие к источнику, свежесть, бренды, очки Hacker News, число источников с той же темой.
3. Лучшие 30 кандидатов оценивает LLM по шкале 1-10 («захочется ли прочитать и переслать»). В модерацию попадают только оценки не ниже `MIN_LLM_SCORE` (по умолчанию 7). Если хороших новостей нет, бот ничего не предлагает.
4. Если LLM недоступна, работает старый строгий набор ключевых слов.

Объём предложений: `DAILY_OFFER_TARGET=12` новостей в день делится между запусками (утро и вечер). Если утром нашлось меньше, недостающее добирается вечером; больше `MAX_OFFERS_PER_RUN` за запуск не предлагается. Если новостей с оценкой не ниже `MIN_LLM_SCORE` не хватает, добор идёт лучшими из тех, у кого оценка не ниже `FALLBACK_LLM_SCORE`. Ниже этой планки бот ничего не добавляет, поэтому 12 в день это цель, а не гарантия. Уже предложенные новости повторно не показываются в течение `MAX_NEWS_AGE_HOURS`.

Оценка LLM и очки HN записываются в `published.json` (`llm_score`, `hn_points`), чтобы позже сопоставить их с просмотрами и пересылками в статистике канала.
